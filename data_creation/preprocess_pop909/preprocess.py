...
def quantize_notes(notes, beat_times, downbeat_idx, beat_in_bar):
...
def align_midi_beats(piece_dir, subfolder, beat_in_bar):
    ...
    vocal_notes = quantize_notes(vocal_notes, midi_beat_times, downbeat_idx, beat_in_bar)
    bridge_notes = quantize_notes(bridge_notes, midi_beat_times, downbeat_idx, beat_in_bar)
    piano_notes = quantize_notes(piano_notes, midi_beat_times, downbeat_idx, beat_in_bar)
    ...
    #     downbeat_idx = 4
    if downbeat_idx == 0:
        downbeat_idx = beat_in_bar
    
    first_beat_tick = (beat_in_bar-downbeat_idx) * DEFAULT_TICKS_PER_BEAT
...
if __name__ == '__main__':
    ...
    os.makedirs(f'{melody_out_dir}/train_3beat', exist_ok=True)
    ...
    for pdir in tqdm(pieces_dir):
        ...
        if mid in train_data:
            ...
            exit(1)
        align_midi_beats(piece_dir, subfolder, beat_in_bar=4)
    print(f"Preprocessed data saved at {melody_out_dir}")


    triple_pieces_dir = pickle.load(open('qual_triple_pieces.pkl', 'rb'))
    for pdir in tqdm(triple_pieces_dir):
        piece_dir = os.path.join(root_dir, pdir)
        mid = f'{pdir}.mid'
        subfolder = 'train_3beat'
        align_midi_beats(piece_dir, subfolder, beat_in_bar=3)
    print(f"Preprocessed data saved at {melody_out_dir}")